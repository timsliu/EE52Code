...
static  enum status  init_Play(enum status cur_status)
{
    ...
    for (i = 0 ; (i < 2); i++)  {
        ...
        if (!end_track)  {
            ...
            /* now read the blocks */
            blocks_read = get_blocks(get_track_block_position() + tot_blocks_read + get_partition_start(), blocks_to_read, buffers[i].p);
    ...
    if (update(buffers[next_buffer].p, buffers[next_buffer].size))  {
        ...
        if (buffers[current_buffer].done && !rpt_play)  {
            ...
            if (!end_play)  {
                ...
                /* now read the blocks */
                blocks_read = get_blocks(start_pos + get_partition_start(), blocks_to_read, buffers[fill_buffer].p);
